...
for (i in sims) {
  ...
  #Here within is taken as pi/4*r^2 area of hemi-sphere and i is area of square r^2,so 
  #when we divide both of them we get pi/4,by multiplying the divided amount with 4 we
  #should get pi value approximately.
...
##We do 100000 simulations to get statistically reliable result
...
text(bp, 0, round(r, 1),cex=1.5,pos=3, col="white")
...
##byrow implies we are asking matrix to be filled with values row wise,first fill
#first row with 3 elements here ,next fill second row so on till 6 rows

Green=Red=Blue=100

GreenOutcome=RedOutcome=BlueOutcome=rep(0, 1000)
...
Pink=Green=Red=Blue=100

PinkOutcome=GreenOutcome=RedOutcome=
  BlueOutcome=rep(0, times=1000)
