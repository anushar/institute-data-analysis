...
library(dummies)
library(class)
knn.ind = data.frame(apply(finaldclean.13[,!(names(knn.final) %in% c("GROWTH_RATE_3YR_14"))],2,dummy))
colnames(knn.ind) = sub(".apply.knn.final.....names.knn.final...in..c..GROWTH_RATE_3YR_14......2..dummy","",colnames(knn.ind))

###split the data on this new set
set.seed(88)
split = sample.split(knn.ind, SplitRatio = 0.90)
Train.13 = subset(knn.ind, split == TRUE)
Train.y = subset(finaldclean.13$GROWTH_RATE_3YR_14,split == TRUE)
Test.13 = subset(knn.ind, split == FALSE)
Test.y = subset(finaldclean.13$GROWTH_RATE_3YR_14, split == FALSE)
